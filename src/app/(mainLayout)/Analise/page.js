...
export default function Analise() {
  ...
  useEffect(() => {
    async function getDevices() {
      try {
        ...
        const dv20 = videoDevicesList.find((d) =>
          d.label.includes("DV20 USB CAMERA")
        );
        if (dv20) {
  ...
  const isFormValid = useMemo(() => {
    const dimensoesValidas =
      ...
      (typeof formData.dimensoes === "object"
        ...
        : !formData.dimensoes
            .toLowerCase()
            .includes("medidas não encontradas"));
  ...
  useEffect(() => {
    ...
    navigator.mediaDevices.enumerateDevices().then((deviceInfos) => {
      ...
      if (videoDevices.length < 2) {
        ...
        setSelectedDevice2({
          posicao: "cima",
          id: videoDevices[cameras[0]].deviceId,
        });
      }
    });
  }, [cameras]);
  ...
  return (
    <>
      {showCameraWarning && (
        <div className="fixed md:top-4 left-1/2 top-24 md:right-[-200px] md:left-auto max-md:min-w-64 transform -translate-x-1/2 z-50 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 rounded-lg shadow-lg max-w-md">
          <div className="flex items-center">
            <svg className="w-6 min-w-6 h-6 mr-3" fill="currentColor" viewBox="0 0 20 20">
      ...
      <div className="w-full h-full flex max-lg:flex-col flex-row py-10 px-5 gap-3">
        <div className="flex flex-col lg:w-1/2 gap-3 bg-white dark:bg-noturno_medio px-7 py-4 rounded-[10px]">
          <ImagemTempoReal
            label={"Altura"}
        ...
        <div className="flex flex-col gap-6 lg:w-1/2 h-full">
          <CardInputs className="h-full w-full flex flex-col justify-between bg-white dark:bg-noturno_medio rounded-[10px] px-7 py-4 gap-5">
            ...
            <Input
              ...
              value={
                formData.dimensoes.largura_cm != 0
                  ? `${formData.dimensoes.comprimento_cm} x ${formData.dimensoes.largura_cm} x ${formData.dimensoes.altura_cm}`
            ...
            <div className="space-y-2 xs:space-y-3">
              <div className="w-full">
                ...
                <Button
                  classes={
                    "text-white p-4 py-4 px-6 bg-gradient-to-r from-azul to-roxo_gradient hover:opacity-90 w-full rounded-xl transition-all duration-200"
                  }
                  onClick={() => {
                    if (!status) {
                      setStatus(!status);
              ...
              <div className="flex flex-row gap-2 xs:gap-3">
                <Link href="/Home" className="flex-1">
                  <Button
                    classes={
                      "text-white p-2 xs:p-3 sm:p-4 py-2 xs:py-3 sm:py-4 bg-gray-400 px-3 xs:px-4 sm:px-6 hover:bg-gray-600 w-full rounded-lg xs:rounded-xl transition-all duration-200 text-sm xs:text-base"
                    }
                  >
                    Cancelar
                  </Button>
                ...
                <Button
                  classes={
                    "p-2 xs:p-3 sm:p-4 py-2 xs:py-3 sm:py-4 px-2 xs:px-3 sm:px-4 bg-azul hover:opacity-90 flex items-center justify-center relative group min-w-[44px] xs:min-w-[50px] sm:min-w-[56px] rounded-lg xs:rounded-xl transition-all duration-200"
                  }
                ...
                >
                  ...
                  >
                    <path
                      d="M0 9.375H9.375V0H0V9.375ZM3.125 3.125H6.25V6.25H3.125V3.125ZM12.5 0V9.375H21.875V0H12.5ZM18.75 6.25H15.625V3.125H18.75V6.25ZM0 21.875H9.375V12.5H0V21.875ZM3.125 15.625H6.25V18.75H3.125V15.625ZM20.3125 12.5H21.875V18.75H17.1875V17.1875H15.625V21.875H12.5V12.5H17.1875V14.0625H20.3125V12.5ZM20.3125 20.3125H21.875V21.875H20.3125V20.3125ZM17.1875 20.3125H18.75V21.875H17.1875V20.3125Z"
                      fill="white"
                    />
                  </svg>
                ...
                <Link
                  ...
                  onClick={() => {
                    ...
                    localStorage.setItem(
                      "ImagemCapturada2",
                      formData.imagens.captura2
                    );
                  }}
                >
                  <Button
                    classes={`text-white p-2 xs:p-3 sm:p-4 py-2 xs:py-3 sm:py-4 px-3 xs:px-4 sm:px-6 w-full rounded-lg xs:rounded-xl transition-all duration-200 text-sm xs:text-base ${
                      isFormValid
                        ? "bg-gradient-to-r from-azul to-roxo_grandient hover:opacity-90 "
